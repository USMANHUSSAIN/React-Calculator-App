...
const SingleCalculation: React.FC<{
  ...
  const handleEdit = (e: React.FormEvent, id: number, currentOperand: string) => {
    e.preventDefault();
    setCalculations(
      calculations.map((calculation) => (calculation.id === id ? { ...calculation, operand: editOperand } : calculation))
    );
  ...
  return (
    ...
    >
      {edit ? (
        ...
        <span className="calculations__single--text" style={{ textDecoration: 'line-through' }}>{calculation.operand}</span>
      ) : (
      ...
      <div>
        ...
        <span className="icon" onClick={() => handleDisabled(calculation.id, calculation.operand, calculation.isDisabled)}>
          <MdDone />
        </span>
      </div>
    </form>
